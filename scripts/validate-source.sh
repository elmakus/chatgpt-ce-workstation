...
env \
  ...
  OPENCODEX_VERSION=0.0.0 OPENCODEX_INTEGRITY="$(python3 -c 'import base64; print("sha512-" + base64.b64encode(b"x" * 64).decode("ascii"))')" OPENCODEX_SHASUM="$(printf 'a%.0s' {1..40})" \
...
[[ -s rootfs/usr/local/bin/workstation-opencodex-proof-config ]] || fail 'OpenCodex proof config helper missing'
