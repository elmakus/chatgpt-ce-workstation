...
Do not assume that editing files on `/workspace` synchronizes them to GitHub. Git add/commit/push are explicit operations.
...
- Repositories live under `/workspace` and physically reside on the Unraid host.
...
- OS package/library/tool -> update `/workspace/chatgpt-ce-workstation/Dockerfile` when that repository is available;
- deployment setting/mount/port/device/environment/secret wiring -> update `/workspace/chatgpt-ce-workstation/compose.yaml`;
